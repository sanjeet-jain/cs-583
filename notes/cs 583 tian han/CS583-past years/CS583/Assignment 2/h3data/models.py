...
class MyGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.g_x = nn.Linear(input_size, hidden_size, bias=False)
    ...
    def forward(self, x, h_prev):
        ...
        # FILL THIS IN
        # ------------
        r = self.r(torch.cat((x, h_prev), 1))
        z = self.z(torch.cat((x, h_prev), 1))
        g = F.tanh(self.g_x(x) + r * self.g_h(h_prev))
        h_new = torch.mul((1.0 - z), g) + torch.mul(z, h_prev)
